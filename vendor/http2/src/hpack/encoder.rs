...
impl Encoder {
    ...
    /// 使用Firefox/NSS的首请求HPACK编码策略。
    pub fn set_firefox_mode(&mut self, enabled: bool) {
        self.firefox_mode = enabled;
    }
    ...
    fn encode_header(&mut self, index: &Index, dst: &mut BytesMut) {
        match *index {
            ...
            Index::Name(idx, _) => {
                ...
                // Firefox对:path字面值使用静态表索引5，Chrome使用索引4。
                let idx = if self.firefox_mode && idx == 4 && header.name().as_slice() == b":path" {
                    5
                } else {
                    idx
                };
            ...
            Index::Inserted(_) => {
                ...
                encode_str(header.name().as_slice(), self.firefox_mode, dst);
                encode_str(header.value_slice(), self.firefox_mode, dst);
            }
...
fn encode_not_indexed(
    ...
    encode_str(value, force_huffman, dst);
}
...
fn encode_not_indexed2(
    ...
    encode_str(name, force_huffman, dst);
    encode_str(value, force_huffman, dst);
}

fn encode_str(val: &[u8], force_huffman: bool, dst: &mut BytesMut) {
    if !val.is_empty() {
        ...
        // Chrome仅在Huffman结果更短时使用压缩，短值保留原文编码。
        if !force_huffman && huff_len >= val.len() {
            dst.truncate(idx);
...
mod test {
    ...
    fn test_encode_method_patch() {
        ...
        assert_eq!(res[0], 0b01000000 | 2);
        assert_eq!(res[1], 5);
        assert_eq!(b"PATCH", &res[2..7]);
    ...
    fn test_encode_indexed_name_literal_value() {
        ...
        assert_eq!(res[0], 0b01000000 | 27);
        assert_eq!(res[1], 0x80 | 2);
    ...
    fn test_encoding_headers_with_same_name() {
        ...
        assert_eq!(&[0x40 | 62, 3], &res[0..2]);
        assert_eq!(b"two", &res[2..]);
        assert_eq!(5, res.len());
