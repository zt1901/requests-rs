...
pub(crate) struct Config {
    ...
    pub defer_initial_settings_ack: bool,
}
    ...
    B: Buf,
{
    pub fn new(codec: Codec<T, Prioritized<B>>, config: Config) -> Connection<T, P, B> {
        ...
        Connection {
            ...
            inner: ConnectionInner {
                ...
                settings: Settings::new(config.settings, config.defer_initial_settings_ack),
                streams,
                #[cfg(feature = "tracing")]
                span,
    ...
    fn poll_ready(&mut self, cx: &mut Context) -> Poll<Result<(), Error>> {
        ...
        if self
            .inner
            .settings
            .should_flush_streams_before_initial_ack()
            && self.inner.streams.has_streams()
        {
            // Chrome先发首个请求帧，再确认服务端初始SETTINGS。
