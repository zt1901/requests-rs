...
pub(crate) struct Settings {
    ...
    /// 客户端首个请求前不抢先确认服务端SETTINGS。
    defer_initial_ack: bool,
}
...
impl Settings {
    pub(crate) fn new(local: frame::Settings, defer_initial_ack: bool) -> Self {
        Settings {
    ...
    pub(crate) fn should_flush_streams_before_initial_ack(&self) -> bool {
        self.defer_initial_ack
        ...
        P: Peer,
    {
        if let Some(settings) = self.remote.clone() {
            if self.defer_initial_ack
                && !self.has_received_remote_initial_settings
                && !streams.has_streams()
            {
                // 等待客户端首个Stream建立，避免ACK跑在浏览器请求帧之前。
