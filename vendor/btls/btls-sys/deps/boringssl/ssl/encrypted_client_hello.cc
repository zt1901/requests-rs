...
static bool setup_ech_grease(SSL_HANDSHAKE *hs) {
  ...
  // Chrome启用普通GREASE并随机选择32字节桶；Firefox关闭普通GREASE并固定最大桶。
