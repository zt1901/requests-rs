...
- 帖子、作者、群组、互动和附件解析。
- 同一群组的首页与全部 GraphQL 分页共享一个 sticky 代理 session；可重试失败时才轮换代理 session，最多即时重试 5 次。
...
测试入口强制使用代理：未传 `--proxy` 时回退到原项目同一份 IPIPGO 默认代理。每个群组采集链路生成一个随机 sticky session，首页与后续分页持续复用；发生可重试失败时才生成新 session。启动时只打印代理主机，不打印认证信息。
